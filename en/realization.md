...
Damba states what it **knows**, not what it imagines. Deterministic, **0 tokens**, no invention: the
...
Damba already reads the endless surface forms of language and reduces them to a small set of **canonical
...
construction**. We realize the fact into a sentence, then **re-read** that sentence with Damba's reader;
...
The same fact can be said in several ways, **all true**. Damba knows that "habiter", "vivre", "résider",
...
On the product side, you can simply ask Damba to **"say it differently"**: it takes its last statement and
offers another attested wording of the same fact, without ever changing its meaning.

## Re-telling a comprehended scene

When a sentence contains a pronoun ("Marc dropped the glass, **it** broke"), Damba first understands **what**
...
uses a pronoun only when it is safe to do so. As everywhere in Realization, nothing is invented: Damba reuses
the words that were actually written, and the produced sentence **re-reads** to the same scene.

## Where it fits

Realization is the **voice** of memory: wherever Damba must answer with a sentence about what it knows
...
> The strength stays **structured and verifiable**: Damba puts into words what it knows, and every
